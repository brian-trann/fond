...
* GET `/recipe/md` - Returns `{markdown : fondMd}` where `fondMd` is a formatted markdown string.
* GET `/recipe/txt` - Returns `{text : fondTxt}` where `fondTxt` is a formatted string for plain text files'
* GET `/recipe/all` - Returns `{recipe : fond, markdown : fondMd, text : fondTxt}` . An object containing the previous 3 properties
